...
%at(x_position, y_position, already catched pokemons locations list, new catched pokemons locations list)
at(3,2,0,[],s).
...
at(X,Y,D,PL,result(A,X,Y,D,S)):-
	p(X,Y,PL,PLNew),
	((canMoveForward(X1,Y1,X,Y,D), A = forward, at(X1,Y1,D,PLNew,S));
	(D1 is D+1, D2 is mod(D1,4), at(X,Y,D2,PLNew,S), A = left);
	(D1 is D-1, D2 is mod(D1,4), at(X,Y,D2,PLNew,S), A = right)).
...
main(S,Depth):-
	call_with_depth_limit(at(2,1,0,[(2,2), (1,2)],S),Depth,R), R \= depth_limit_exceeded. 
main(S,Depth):-
	call_with_depth_limit(at(2,1,1,[(2,2), (1,2)],S),Depth,R), R \= depth_limit_exceeded.
main(S,Depth):-
	call_with_depth_limit(at(2,1,2,[(2,2), (1,2)],S),Depth,R), R \= depth_limit_exceeded.
main(S,Depth):-
	call_with_depth_limit(at(2,1,3,[(2,2), (1,2)],S),Depth,R), R \= depth_limit_exceeded.
